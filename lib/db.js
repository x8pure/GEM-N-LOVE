...
export function setMemoryDb(newDb, persist = true) {
...
function empty() {
  return {
    ...
    meta: { createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), version: 1, seq: { product: 0, order: 0 } }
  };
}

export function load() {
  if (db && Array.isArray(db.products)) return db;
  try {
