...
export function setMemoryDb(newDb, persistLocal = true) {
  if (newDb && typeof newDb === 'object') {
    if (db && typeof db === 'object') {
      if (Array.isArray(db.posSales) && (!Array.isArray(newDb.posSales) || newDb.posSales.length < db.posSales.length)) {
        newDb.posSales = db.posSales;
      }
      if (Array.isArray(db.orders) && (!Array.isArray(newDb.orders) || newDb.orders.length < db.orders.length)) {
        newDb.orders = db.orders;
      }
