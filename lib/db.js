...
export function setMemoryDb(newDb, persistLocal = true) {
  if (newDb && typeof newDb === 'object') {
    ...
    if (db && typeof db === 'object') {
      ...
      curPos.forEach((s) => {
        if (s && s.id && !posMap.has(s.id)) posMap.set(s.id, s);
      });
      ...
      // 3. Merge wheelIds settings
      if (!newDb.settings) newDb.settings = {};
      if ((!Array.isArray(newDb.settings.wheelIds) || newDb.settings.wheelIds.length === 0) && Array.isArray(db.settings?.wheelIds) && db.settings.wheelIds.length > 0) {
