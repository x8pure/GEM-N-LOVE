...
export function getFirebaseStorage() {
  ...
  try {
    ...
    if (!app) return null;
    firebaseStorage = getStorage(app, DEFAULT_CONFIG.storageBucket);
    return firebaseStorage;
  } catch (err) {
    console.error('[Firebase Storage] Get storage error:', err);
    return null;
  }
}

export async function uploadToFirebaseStorage(fileName, buffer, mimeType = 'image/webp') {
  try {
    const storage = getFirebaseStorage();
    if (!storage || !fileName || !buffer) return null;
    const cleanPath = `products/${fileName.replace(/[^a-zA-Z0-9._-]/g, '_')}`;
    ...
    console.log(`[Firebase Storage] Uploaded: ${fileName} -> ${downloadUrl}`);
    return downloadUrl;
  } catch (err) {
    console.error('[Firebase Storage] Upload failed for', fileName, err);
    return null;
  }
}
