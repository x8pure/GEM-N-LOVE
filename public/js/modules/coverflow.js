...
export function destroyCoverflow() {
  if (typeof activeCoverflowCleaner === 'function') {
    try {
      activeCoverflowCleaner();
    } catch (e) {
      console.warn('Coverflow cleanup error:', e);
    }
    activeCoverflowCleaner = null;
  }
}

/* ================= DEPTHDECK COVERFLOW — Perspective Fan & Kinetic Inertia Stage (2026) ================= */
export async function initCoverflow(stage) {
  if (!stage) return;
  
  // Clean up any existing active instance
  destroyCoverflow();
  ...
  // Make sure stage is attached to DOM
  ...
  stage.innerHTML = `
    ...
    <button class="cf-arrow cf-prev" id="cf-prev" aria-label="${t('cf.prev') || 'Önceki'}">←</button>
    <button class="cf-arrow cf-next" id="cf-next" aria-label="${t('cf.next') || 'Sonraki'}">→</button>
  ...
  function tint(p) {
    ...
    const key = p.image;
    const apply = () => {
      const col = colorCache[key];
      if (col && amb) amb.style.background = `radial-gradient(circle, ${col}, rgba(255,228,225,0) 70%)`;
    };
    ...
    im.onload = () => {
      try {
        ...
        if (n > 0) {
          colorCache[key] = `rgba(${Math.round(r / n)}, ${Math.round(g / n)}, ${Math.round(b / n)}, 0.28)`;
          apply();
        }
      } catch {}
    };
  ...
  function layout(t) {
    ...
    if (best !== focusIdx && best >= 0 && prods[best]) {
      focusIdx = best;
  ...
  cards.forEach((card, i) => {
    ...
    card.addEventListener('click', (e) => {
      ...
      if (Math.abs(d) > 0.4) {
        ...
        if (slug && typeof window.openSpatialCardZoom === 'function') {
          window.openSpatialCardZoom(slug, card);
        }
      }
    });
  });

  const nextBtn = $('#cf-next', stage);
  const prevBtn = $('#cf-prev', stage);
